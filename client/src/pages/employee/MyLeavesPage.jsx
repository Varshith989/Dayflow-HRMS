...
const MyLeavesPage = () => {
  ...
  const [leaveBalance, setLeaveBalance] = useState(user?.leaveBalance || { paid: 12, sick: 8, unpaid: 0 });
      ...
      case 'Pending':
        return (
          <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-bold bg-amber-500/10 text-amber-300 border border-amber-500/20">
            <span className="w-2 h-2 rounded-full bg-amber-400 animate-pulse"></span> Pending Review
          </span>
        );
      case 'Rejected':
        return (
        ...
        <div className="p-5 rounded-2xl bg-slate-900/80 border border-slate-800 flex items-center justify-between">
          <div>
            ...
            <div className="text-2xl font-black text-emerald-400 mt-1">
              {leaveBalance?.paid || 0} <span className="text-xs font-normal text-slate-400">days</span>
            </div>
            <span className="text-[11px] text-slate-500 mt-0.5 block">Full pay compensation</span>
          </div>
        ...
        <div className="p-5 rounded-2xl bg-slate-900/80 border border-slate-800 flex items-center justify-between">
          <div>
            ...
            <div className="text-2xl font-black text-brand-400 mt-1">
              {leaveBalance?.sick || 0} <span className="text-xs font-normal text-slate-400">days</span>
            </div>
            <span className="text-[11px] text-slate-500 mt-0.5 block">Medical & health quota</span>
          </div>
        ...
        <div className="p-5 rounded-2xl bg-slate-900/80 border border-slate-800 flex items-center justify-between">
          <div>
            ...
            <div className="text-2xl font-black text-amber-400 mt-1">
              {stats.pending}{' '}
              <span className="text-xs font-normal text-slate-400">awaiting review</span>
            </div>
            <span className="text-[11px] text-slate-500 mt-0.5 block">HR evaluation queue</span>
          </div>
      ...
      <div className="p-4 rounded-2xl bg-slate-900/80 border border-slate-800 flex flex-col sm:flex-row items-center justify-between gap-4">
        ...
        <div className="relative w-full sm:w-72">
          ...
          <input
            ...
            className="w-full pl-10 pr-4 py-2 bg-slate-950/80 border border-slate-800 rounded-xl text-xs text-white placeholder-slate-500 focus:outline-none focus:ring-1 focus:ring-brand-500"
          />
        </div>
      </div>
                ...
                {filteredLeaves.map((l) => (
                  <tr key={l._id} className="hover:bg-slate-850/50 transition-colors">
                    ...
                    <td className="px-6 py-4 font-mono text-slate-200">
                      <div>
                        {l.startDate} <span className="text-slate-500">to</span> {l.endDate}
                      </div>
                    </td>
                    ...
                    <td className="px-6 py-4 text-slate-400 max-w-xs">
                      {l.adminComment ? (
                        <div className="flex items-start gap-1.5 text-slate-300 bg-slate-950/60 p-2 rounded-xl border border-slate-800">
                        ...
                        <span className="text-slate-500 italic">No comments</span>
                      )}
                    </td>
                  </tr>
                ))}
